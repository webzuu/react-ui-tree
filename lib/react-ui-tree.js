...
export default class UITree extends Component {
  ...
  drag = e => {
    if (this._start) {
      let startIndex = this.state.tree.getIndex(this.dragging.id);
      let startParent = this.state.tree.getIndex(startIndex.parent);
      this.setState({
        dragging: this.dragging,
        draggingFrom: startParent.node
      });
      this._start = false;
    }
  ...
  dragEnd = () => {
    const { tree, dragging, draggingFrom } = this.state;

    let node = tree.getIndex(dragging.id);
    let parentNode = node ? tree.getIndex(node.parent).node : null;

    let moveData = node && parentNode ? {
      subject: node.node,
      from: draggingFrom,
      to: parentNode,
  ...
  toggleCollapse = nodeId => {
    ...
    if (node.collapsed && this.props.onCollapse) {
      this.props.onCollapse(node);
    }

    if (!node.collapsed && this.props.onExpand) {
